...
{
    //  class is loaded before the default load time, so references are ensured to be set up before being called.
    // This should only be placed on one GameObject.
    public class References : MonoBehaviour, IReferences
    {
        [field: SerializeField] public UIReferences UI { get; private set; }
